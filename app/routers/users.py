...
def list_users(
    ...
    return [
        UserOut(
            id=r[0], username=r[1], email=r[2], hashed_password=r[3],
            role=r[4], is_active=r[5], created_at=r[6],
        )
        for r in rows
    ]
...
def get_user(user_id: int, current_user: CurrentUser = Depends(get_current_user)) -> UserOut:
    ...
    return UserOut(
        ...
        role=row[4], is_active=row[5], created_at=row[6],
    )
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                "VALUES (%s, %s, %s, %s) "
                "RETURNING id, username, email, hashed_password, role, is_active, created_at",
...
def delete_user(user_id: int, current_user: CurrentUser = Depends(get_current_user)) -> None:
...
def get_user_logs(
    ...
    return [
        AuditLogOut(id=r[0], user_id=r[1], action=r[2], ip_address=r[3], created_at=r[4])
        for r in rows
    ]
